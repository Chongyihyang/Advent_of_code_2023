pub mod aoc11_1{
    ...
    pub fn find_galaxies(input: &str, cols: &Vec<u32>, rows: &Vec<u32>, number: u32) -> Vec<Coordinates> {
        let mut coords = vec![];
        for (y, line) in input.lines().rev().enumerate(){
            for (x, char) in line.chars().enumerate(){
                if char == '#' {
                    let (mut x, mut y) = (x as u32, y as u32);
                    x += cols.iter().filter(|&q| q < &x).count() as u32 * (number - 1);
                    y += rows.iter().filter(|&q| q < &y).count() as u32 * (number - 1);
    ...
    pub fn main11_1(){
        let contents = read("AOC11.txt");
        let (cols, rows) = (expand_columns(&contents), expand_rows(&contents));
        let galaxies = find_galaxies(&*contents, &cols, &rows, 2);
...
pub mod aoc11_2{
    use crate::AOC11::aoc11_1::{expand_columns, expand_rows, find_galaxies, find_permutations};
    use crate::python_builtins::builtins::read;

    pub fn main11_2(){
        let mut contents = read("AOC11.txt");
        let (cols, rows) = (expand_columns(&contents), expand_rows(&contents));
        let galaxies = find_galaxies(&*contents, &cols, &rows, 1000000);
        let steps = find_permutations(1, galaxies.len() as u32, galaxies);
        println!("{steps}");
    }
}